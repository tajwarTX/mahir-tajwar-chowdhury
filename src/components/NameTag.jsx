...
const visibleLetterStyle = {
  opacity: 1,
  color: 'inherit',
  WebkitTextFillColor: 'inherit',
};

function LetterReveal({ text, revealTrigger }) {
  ...
  useLayoutEffect(() => {
    ...
    setVisibleMask(Array(count).fill(false));

    if (rafRef.current) cancelAnimationFrame(rafRef.current);

    const delays = Array.from({ length: count }, () => Math.random() * REVEAL_DURATION);
    ...
    return (
      <span aria-hidden="true">
        {text.split('').map((letter, i) => (
          <span key={`idle-${i}`} style={hiddenLetterStyle}>
            {letter}
          </span>
        ))}
      </span>
  ...
  return (
    <span>
      {letters.map((letter, i) => (
        <span
          key={i}
          style={visibleMask[i] ? visibleLetterStyle : hiddenLetterStyle}
        >
          {letter}
        </span>
      ))}
...
export default function NameTag({ scrollRootRef }) {
  ...
  useEffect(() => {
    const el = nameAreaRef.current;
    if (!el) return;

    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting && !hasStartedRevealRef.current) {
      ...
      <div className="mt-6 flex justify-center z-10">
        ...
        >
          ...
          >
            ...
            <div
              ref={nameAreaRef}
              className="px-2 md:px-4 h-[115px] md:h-[142px] flex flex-col items-center justify-center relative z-20 pointer-events-none"
            >
              <div 
